...
public class SmsReceiverActivity extends Activity {
	...
	public void onNewIntent(Intent intent) {
		...
		if((intent.getStringExtra("start?").toString()).equals("getMore")){
			int sentP = intent.getIntExtra("sentPackets", 1000);
			String resend = "%&resend ";
			Toast.makeText(getBaseContext(), "Received Packet up to "+intent.getIntExtra("sentPackets", 1000), Toast.LENGTH_SHORT).show();
			for(int i = sentP - messageSize-1; i<=sentP; i++){
				if(al.get(Integer.toString(i)) == null){
					resend += Integer.toString(i) + " ";
				}
			}
		...
		if ((intent.getStringExtra("start?").toString())
			...
			Toast.makeText(getBaseContext(), "Received Packet #"+intent.getIntExtra("packetNum", 1000), Toast.LENGTH_SHORT).show();
